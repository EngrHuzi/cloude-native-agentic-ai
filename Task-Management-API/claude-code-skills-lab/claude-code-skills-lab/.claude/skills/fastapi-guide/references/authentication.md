...
**Never store plaintext passwords.** Secure applications store cryptographic hashes that cannot be reversed. If a database is compromised, attackers gain only useless hash strings rather than functional credentials.

**Argon2 is the current gold standard** for password hashing because:
- **Memory-hard algorithm**: Resistant to GPU-based and ASIC attacks
- **Winner of the 2015 Password Hashing Competition**
- **Configurable difficulty**: Security can be upgraded over time as hardware improves
- **Three variants**: Argon2d (GPU-resistant), Argon2i (side-channel resistant), Argon2id (hybrid - recommended)

While bcrypt is still secure, Argon2 provides better protection against modern attack methods.
...
```bash
uv add "python-jose[cryptography]" "pwdlib[argon2]" python-multipart
```
...
|-----------|------------|
| Never store plaintext | Always hash passwords before database storage |
| Hide hashes in responses | Return only safe fields (id, email) - never return hashed_password |
| Prevent email enumeration | Check for duplicates before user creation |
| Use modern algorithms | Prefer Argon2id over MD5, SHA, or even bcrypt |
| Proper field naming | Use `hashed_password` not `password` to avoid confusion |
| Validate at boundaries | Only validate user input and external APIs, trust internal code |

## Common Security Pitfalls to Avoid

❌ **DON'T:**
- Store passwords directly instead of hashes
- Return password hashes in API responses
- Use weak algorithms like MD5 or SHA-1 for passwords
- Name database fields `password` when they store hashes
- Hash passwords on the client side (always hash server-side)
- Use the same salt for all passwords (modern libraries handle this automatically)

✅ **DO:**
- Use Argon2 or bcrypt for password hashing
- Store only hashed passwords in the database
- Return minimal user data (id, email, username) in responses
- Implement rate limiting on authentication endpoints
- Use HTTPS in production
- Keep your SECRET_KEY truly secret and rotate it periodically
